...
APP.config(function ($routeProvider, $locationProvider) {
		/*for (var i=0; i<routes.length; i++) {
			...
			$routeProvider.when('/' + route.URLSegment, {
				templateUrl: '/?view=' + route.URLSegment,
				controller: 'MainCtrl'
			});
		}
		...
		$routeProvider.otherwise({
			templateUrl: '/?view=page-not-found',
			controller: 'MainCtrl'
		});
...
(function($){$(function(){
    Scrollzone = {
        ...
        onWindowResize: function() {
            ...
            var mainHeight          = this.main.height('auto').height();
            var mainOuterHeight     = this.main.height('auto').outerHeight();
            var scrollzoneHeight    = windowHeight - headerHeight;

            // scrollzone
            this.scrollzone.height(scrollzoneHeight);

            // main
            var extraMainHeight = 0;
            var temp = scrollzoneHeight - footerHeight - mainOuterHeight;
            if (temp > 0) extraMainHeight = temp;
            
            this.main.height(mainHeight + extraMainHeight);
        }
    };
  return Scrollzone.init();
});})(jQuery);
/**
 * SmoothScrolling
 */
